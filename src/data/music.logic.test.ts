...
describe('classement des musiques', () => {
  it('regroupe les préfixes fournis et toutes les variantes Umoir', () => {
    ...
    expect(musicZone(track('Airin_Castle_Indoor'))?.title).toEqual({ fr: 'Irene', en: 'Irene' });
  ...
  it('replace les banques récentes sous Zones et conserve toutes les pistes inconnues', () => {
    ...
    tracks.push(track('Unknown_Main_NM'), track('ZL1_Main_1'));
    ...
    expect(children.find(c => c.id === 'other')?.tracks[0].name).toBe('Unknown_Main_NM');
